...
from mira.helpers.const import UUID_DEVICE_NAME, UUID_MODEL_NUMBER, UUID_MANUFACTURER, UUID_READ, UUID_WRITE, MAGIC_ID, \
    TIMER_RUNNING, TIMER_PAUSED, OUTLET_RUNNING, OUTLET_STOPPED
from mira.helpers.generic import _split_chunks, _format_bytearray, _bits_to_list, _convert_temperature_reverse, \
    _get_payload_with_crc, _convert_temperature
...
class Connection:
    ...
    def connect(self):
        self._peripheral = bluetooth.async_ble_device_from_address(self._hass, self._address, connectable=True)
