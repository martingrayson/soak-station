...
class Notifications:
    def __init__(self, *, model: SoakStationData = None, metadata: SoakStationMetadata = None, is_pairing=False):
        self._model = model
    ...
    def _handle_device_state(self, slot, payload):
        ...
        timer_state = TIMER_STATE_MAP.get(payload[1])
    ...
    def _handle_preset_details(self, slot, payload):
        if self._metadata:
            slot = payload[0]
            target_temp = _convert_temperature_reverse(payload[1:3])
            duration = payload[4]
            outlet_flags = _bits_to_list(payload[5], 8)
